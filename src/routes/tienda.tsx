...
export const Route = createFileRoute("/tienda")({
  head: () => ({
    meta: [
      ...
      { name: "description", content: "Suplementos, snacks saludables e infusiones seleccionados por Melina Oviedo." },
    ...
    <SiteLayout>
      <section className="mx-auto max-w-7xl px-4 py-16 sm:px-6 md:py-24 lg:px-8">
        <Reveal className="mx-auto mb-10 max-w-2xl text-center">
          ...
          <p className="mt-3 text-muted-foreground">Productos saludables seleccionados con cuidado.</p>
        </Reveal>
        ...
        {filtered.length === 0 ? (
          <p className="py-16 text-center text-muted-foreground">Pronto vas a encontrar productos disponibles aquí.</p>
        ) : (
                ...
                <div className="flex h-full flex-col overflow-hidden rounded-3xl border border-border bg-card shadow-soft transition-transform hover:-translate-y-1">
                  ...
                  <div className="flex flex-1 flex-col p-6">
                    ...
                    <div className="mt-4 flex items-center justify-between">
                      <span className="font-display text-xl font-semibold text-primary">{formatPrice(p.price)}</span>
                      <Button size="sm" className="rounded-full" disabled={p.stock <= 0} onClick={() => handleAdd(p)}>
